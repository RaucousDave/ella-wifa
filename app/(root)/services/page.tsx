    ...
    <PageWrapper>
      ...
      <section className="bg-background text-background py-20 px-6 md:px-16">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-12">
          {page.map((service) => (
            <div
              key={service.title}
              className="bg-background p-6 rounded-xl text-primary-light   border border-primary-light select-none transition"
            >
              <h3 className="font-heading text-center sm:text-left tracking-tight text-2xl mb-3">
                {service.title}
              </h3>
              <p className="font-body tracking-tight text-center sm:text-left text-sm text-gray-200 leading-relaxed">
                {service.description}
              </p>
            </div>
          ))}
      ...
      <section className="bg-background text-foreground px-6 md:px-16 pb-24 text-center">
        <FadeIn delay={0}>
          ...
          <p className="font-body tracking-tight text-white max-w-2xl mx-auto text-sm md:text-lg mb-6">
            Whether it’s a brand audit, content templates, or consultation — I
            can create something unique just for your vision.
          </p>
          <Link
            href="/contact"
            className="bg-primary-light hover:bg-primary-dark text-background px-6 py-3 rounded tracking-tight font-body hover:bg-lilac transition"
          >
            Let’s Talk
          </Link>
        </FadeIn>
